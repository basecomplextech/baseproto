...
func (w *messageWriter) write(def *model.Definition) error {
	...
	if err := w.hasFields(msg); err != nil {
		return err
	}
	if err := w.methods(msg); err != nil {
		return err
	}
	if err := w.parse(msg); err != nil {
		return err
	}
	return nil
}
...
func (w *messageWriter) def(msg *golang.Message) error {
	...
	w.Writef(`type %v struct {`, msg.Name)
	w.Write(`msg baseproto.Message`)
	w.Write(`}`)
...
func (w *messageWriter) parse(msg *golang.Message) error {
...
func (w *messageWriter) field(msg *golang.Message, field *golang.MessageField) error {
	...
	w.Writef(`func (m %v) %v() %v {`, msg.Name, field.Name, typ.OutputName())
	typ.GetField(w, field.Tag)
	w.Writef(`}`)
	w.Line()
	return nil
}
...
func (w *messageWriter) hasFields(msg *golang.Message) error {
	for _, field := range msg.Fields {
