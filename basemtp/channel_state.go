...
func openChannelState(conn internalConn, client bool, msg pmtp.ChannelOpen) *channelState {
	id := msg.Id()
	window := msg.Window()
...
func (s *channelState) receiveMessage(msg pmtp.Message) status.Status {
	code := msg.Code()
	...
	return mtpErrorf("message must be handled by connection, code=%v", code)
}

func (s *channelState) receiveClose(msg pmtp.ChannelClose) status.Status {
	if s.closed.Load() {
		return status.OK
	}
...
func (s *channelState) receiveData(msg pmtp.ChannelData) status.Status {
	data := msg.Data()
	_, _ = s.recvQueue.Write(data) // ignore end and false, receive queues are unbounded
	return status.OK
}

func (s *channelState) receiveWindow(msg pmtp.ChannelWindow) status.Status {
...
func (s *channelState) decrementSendWindow(ctx async.Context, data []byte) status.Status {
	...
	n := len(data)
	if n > math.MaxInt32 {
		return mtpErrorf("message too large, size=%d", n)
	}
	size := int32(n)
