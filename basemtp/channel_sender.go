...
func (s channelSender) sendOpen(ctx async.Context, data []byte) status.Status {
	...
	w := pmtp.NewMessageWriterBuffer(buf)
	msg, err := pmtp.BuildChannelOpen(w, s.ch.id, data, s.ch.initWindow)
	if err != nil {
		return mtpError(err)
	}
...
func (s channelSender) sendClose(ctx async.Context, data []byte) status.Status {
	...
	w := pmtp.NewMessageWriterBuffer(buf)
	msg, err := pmtp.BuildChannelClose(w, s.ch.id, data)
	if err != nil {
		return mtpError(err)
	}
...
func (s channelSender) sendData(ctx async.Context, data []byte) status.Status {
	...
	w := pmtp.NewMessageWriterBuffer(buf)
	msg, err := pmtp.BuildChannelData(w, s.ch.id, data)
	if err != nil {
		return mtpError(err)
	}
...
func (s channelSender) sendWindow(ctx async.Context, delta int32) status.Status {
	...
	w := pmtp.NewMessageWriterBuffer(buf)
	msg, err := pmtp.BuildChannelWindow(w, s.ch.id, delta)
	if err != nil {
		return mtpError(err)
	}
