...
type internalChannel interface {
	...
	receive(msg pmtp.Message) status.Status
...
func openChannel(conn internalConn, client bool, msg pmtp.ChannelOpen) *channel {
...
// receive is called by the connection to receive a message.
func (ch *channel) receive(msg pmtp.Message) status.Status {
	s := ch.acquire()
	defer ch.release()
