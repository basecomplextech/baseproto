...
func (c *conn) handshakeAsClient() status.Status {
	...
	if line != ProtocolLine {
		return mtpErrorf("invalid protocol, expected %q, got %q", ProtocolLine, line)
	}
	...
	if ok := resp.Ok(); !ok {
		return mtpErrorf("server refused connection: %v", resp.Error())
	}

	// Check version
	v := resp.Version()
	if v != pmtp.Version_Version10 {
		return mtpErrorf("server returned unsupported version %d", v)
	}

	// Init compression
	comp := resp.Compression()
	switch comp {
	case pmtp.ConnectCompression_None:
	case pmtp.ConnectCompression_Lz4:
	...
	default:
		return mtpErrorf("server returned unsupported compression %d", comp)
	}
...
func (c *conn) handshakeAsServer() status.Status {
	...
	if !ok {
		resp, err := pmtp.BuildConnectError("unsupported protocol versions")
		if err != nil {
			return mtpError(err)
		}
	...
	// Select compression
	comp := pmtp.ConnectCompression_None
	comps := req.Compression()
	for i := 0; i < comps.Len(); i++ {
		c := comps.Get(i)
		if c == pmtp.ConnectCompression_Lz4 {
			comp = pmtp.ConnectCompression_Lz4
			break
		}
	}

	// Write response
	resp, err := pmtp.BuildConnectResponse(pmtp.Version_Version10, comp)
	if err != nil {
		return mtpError(err)
	}
